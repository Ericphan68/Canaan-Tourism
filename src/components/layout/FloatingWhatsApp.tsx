...
export function FloatingWhatsApp() {
  ...
  return (
    <a
      ...
      aria-label="Nhắn WhatsApp cho Canaan Tourism"
      className="group fixed bottom-20 right-4 z-40 inline-flex items-center gap-2 rounded-full bg-[#25D366] py-3 pl-3 pr-3 text-white shadow-console transition-all duration-300 ease-canaan hover:pr-5 sm:bottom-6 sm:right-6"
    >
      <IconWhatsapp className="h-6 w-6" />
      <span className="hidden max-w-0 overflow-hidden whitespace-nowrap text-sm font-semibold transition-all duration-300 ease-canaan group-hover:max-w-[10rem] sm:inline">
        Tư vấn WhatsApp
      </span>
    </a>
  );
