...
export function ContactForm() {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-ink-muted">Điền form, Canaan sẽ phản hồi qua email hoặc điện thoại.</p>
      ...
      <div className="mt-5">
        {done ? <Success title="Đã gửi! Canaan sẽ phản hồi sớm." /> : <Button type="submit" variant="primary" size="lg">Gửi tin nhắn</Button>}
      </div>
    </form>
  );
}
...
export function CallbackForm() {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-white/70">Để lại số, chuyên viên Canaan sẽ gọi cho bạn.</p>
      ...
      <div className="mt-4">
        {done ? (
          <div className="flex items-center gap-2 rounded-xl bg-white/10 px-4 py-3 text-sm font-medium text-champagne-400">
            <IconCheck className="h-5 w-5" /> Đã ghi nhận. Canaan sẽ gọi lại cho bạn.
          </div>
        ) : (
