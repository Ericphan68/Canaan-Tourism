  ...
  for (let d = 1; d <= n; d++) {
    if (d === 1) {
      days.push({
        ...
        description: fromVn
          ? 'Tập trung tại sân bay, làm thủ tục và khởi hành. Trưởng đoàn Canaan đồng hành cùng quý khách.'
...
const authored: Record<string, Partial<TourDetail>> = {
  'dubai-abu-dhabi-6n5d-tu-tphcm': {
    ...
    visa: 'Bao gồm visa UAE (Canaan xử lý hồ sơ).',
    faqs: [
      { question: 'Tour đã bao gồm visa UAE chưa?', answer: 'Đã bao gồm. Quý khách chỉ cần cung cấp hộ chiếu và ảnh theo hướng dẫn, Canaan lo phần còn lại.' },
...
export function getTourDetail(tour: Tour): TourDetail {
  ...
  return {
    ...
    visa: a.visa ?? 'Canaan tư vấn và hỗ trợ hồ sơ visa phù hợp hành trình.',
